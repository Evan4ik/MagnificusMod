    ...
    class BossOpponents
    {
        ...
        public class MagnusSequencer : Part1BossBattleSequencer
        {
            ...
            public IEnumerator fixCardBacks()
            {
                ...
                {
                    GameObject.Find("SelectableCardArray").transform.GetChild(i).gameObject.GetComponent<BoxCollider>().enabled = false;
        ...
        public class LonelyMageSequencer : Part1BossBattleSequencer
        {
            ...
            public IEnumerator shuffleBoard(bool firstTime = true)
            {
                ...
                if (firstTime)
                {
                    Tween.LocalPosition(GameObject.Find("SelectableCardArray").transform, new Vector3(-0.72f, -75f, 4.059f), 5f, 0.4f);
                ...
                {
                    int safety = -50;
                    if (slot.opposingSlot.Card != null) safety = slot.opposingSlot.Card.Attack;
                    if (slot.Card != null) safety += 50;

                    if (safety < safestSlot)
                    {
                        emptySlot = slot.Index;
                        safestSlot = safety;
                    }
                ...
                CardModificationInfo tempMod = new CardModificationInfo();
                tempMod.singletonId = "DECKCARD";
                if (Singleton<BoardManager>.Instance.OpponentSlotsCopy[emptySlot].Card != null) { yield return Singleton<BoardManager>.Instance.opponentSlots[emptySlot].Card.Die(false); }
